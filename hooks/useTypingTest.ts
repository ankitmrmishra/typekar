...
export function useTypingTest(defaultWordCount: number = 200) {
  ...
  // **MODIFIED:** This useEffect now regenerates the paragraph whenever the word count (`end`) changes.
  // This ensures a new test is created on reset.
  useEffect(() => {
    const randomIndex = Math.floor(Math.random() * texts.length);
    const randomParagraph = texts[randomIndex];
    const newPara = randomParagraph
      .split(" ")
      .slice(start, end)
      .join(" ")
      .toLowerCase();
    setParagraph(newPara);
  }, [end, start]); // Reruns when 'end' is changed by resetTest
  ...
  if (startTime && endTime) {
    ...
    accuracy =
      completedWords.length > 0
        ? Math.round((correctWords.length / completedWords.length) * 100)
        : 0;
  }
   ...
   */
  // **MODIFIED:** This function now correctly resets all state for a new test.
  const resetTest = (newEnd: number) => {
    ...
    setStartTime(null);
    setEndTime(null);
    // The paragraph will be regenerated by the useEffect that depends on 'end'
  };
